...
struct _Head_base<_Idx, _Head, true> : public _Head
{
    ...
    DEVICE_FUNC void _M_swap_impl(_Head&)
    { /* no-op */
    }
};
...
{
protected:
    DEVICE_FUNC void _M_swap_impl(_Tuple_impl&)
    { /* no-op */
    }
};
                                            ...
                                            private _Head_base<_Idx, _Head, is_empty<_Head>::value>
{
    ...
    explicit DEVICE_FUNC _Tuple_impl(_UHead&& __head, _UTail&&... __tail)
        : _Inherited(forward<_UTail>(__tail)...)
    ...
    DEVICE_FUNC _Tuple_impl(_Tuple_impl&& __in)
        : _Inherited(move(__in._M_tail()))
        , _Base(forward<_Head>(__in._M_head()))
    {
    }
...
{
    static size_t const value = sizeof...(_Elements);
};

template <typename... _Elements>
size_t const tuple_size<tuple<_Elements...>>::value;
...
struct __tuple_compare<0, __i, __j, _Tp, _Up>
{
    ...
    DEVICE_FUNC static bool __less(_Tp const& __t, _Up const& __u)
    {
        return ((get<__i>(__t) < get<__i>(__u))
            || !(get<__i>(__u) < get<__i>(__t)) && __tuple_compare<0, __i + 1, __j, _Tp, _Up>::__less(__t, __u));
    }
};
