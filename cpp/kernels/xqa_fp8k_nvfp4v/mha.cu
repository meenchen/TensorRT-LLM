...
__constant__ constexpr uint32_t cacheVTileSeqLen = 32;
    ...
    || __CUDA_ARCH__ == 1030 || __CUDA_ARCH__ == 1100
...
struct alignas(128) SharedMem
{
    ...
    using KSfSmemBuffer
        = Array2D<uint32_t, warpTile.x, exactDiv(kHeadPartBytes / KCacheElemConverter::QuantVectorSize, grainBytesSf)>;
    ...
    using VSfSmemBuffer = Array2D<uint32_t, cacheVTileSeqLen,
        exactDiv((grpLoadV ? headElems : warpTile.x) / VCacheElemConverter::QuantVectorSize, grainBytesSf)>;
    using VSfSmemBufferPlain = Array2D<__nv_fp8_e4m3, cacheVTileSeqLen,
    ...
    SMemWarpRowMax ctaRowMax[ctaShapeInWarps.y]; // just a hint, no strict protection required if you don't care
    ...
    Barrier ctaRowMaxBwdBarriers[ctaShapeInWarps.y][ctaShapeInWarps.x]; // xFwdBarriers+ctaRowMaxBwdBarriers
                                                                        // protects ctaRowMax
#endif
...
__device__ inline void storeReorderedXTile(Warp const& warp, SharedMem::XSmemBuffer& dst, GemmOutRegTile const& src)
{
    static_assert(sizeof(dst) == sizeof(src) * warp_size);
    uint32_t const lane = laneId();
...
template <uint32_t kEx, uint32_t mnEx, bool transOuter, bool transInner, bool is4BitElem, uint32_t srcRows,
    uint32_t srcCols>
...
template <uint32_t kEx, uint32_t mnEx, uint32_t dstRows, uint32_t dstCols, bool transArr2D, bool transInstInMatOuter,
    bool transInstInMatInner, bool is4BitElem, uint32_t srcRows, uint32_t srcCols /*in LdGrain*/>
...
#pragma unroll
                            ...
                            {
                                auto data = convertKCacheWordToF16<InputElem, KElemType>(kSliceOrig(m, n).data[i][j]);
#if ENABLE_4BIT_K_CACHE
...
#pragma unroll
        for (uint32_t j = 0; j < nbSfPrefetchBuffers; j++)
        {
            ...
            vSfPrefetch(i, j) = reinterpret_cast<uint32_t const&>(vSf.template at(sfRowIdx, j));
        }
    }
#endif
...
#pragma unroll
    for (uint32_t s = 0; s < gemmKSplit; s++)
    {
        ...
        Array2D<InstInMat<kEx, mnEx>, xSliceRows, xSliceCols> xSlice
            = loadMatrix<kEx, mnEx, xSliceRows, xSliceCols, false, false, false, false>(warp, x, 0u, colBeg);
        if (!enableMicroFastPath || !skipXRowRescale)
        {
#pragma unroll
...
__device__ inline void addAttentionSinksSpecDec(
    ThrdRegRowMax& globalRowSum, ThrdRegRowMax const globalRowMax, float const* attentionSinks, uint32_t headGrpSize)
{
    for (uint32_t i = 0; i < globalRowSum.size; i++)
    {
...
#endif
#endif
        uint32_t const batchSize, float kCacheScale, float const* kScalePtr, float vCacheScale, float const* vScalePtr,
        uint32_t k_stride_page, uint32_t k_stride_token, uint32_t k_stride_head, uint32_t v_stride_page,
        uint32_t v_stride_token, uint32_t v_stride_head, uint32_t k_sf_stride_page, uint32_t k_sf_stride_token,
        uint32_t k_sf_stride_head, uint32_t v_sf_stride_page, uint32_t v_sf_stride_token, uint32_t v_sf_stride_head,
        uint32_t* __restrict__ semaphores = nullptr, void* __restrict__ scratch = nullptr)
{
    ...
    if (warpIdx.z == 0)
    {
        ...
        if (isFullTile)
        {
            copyHeadsAsync<PaddedInputHead, warpTile.y, ctaShapeInWarps.x, grainBytes, grainBytes, qkSwizzle, true,
                warpTile.y>(warpIdx.x, smem.q[warpIdx.y][0], src, nbValidHeadTokens, localQHeadTokenIdxMap);
        }
        else
        {
            copyHeadsAsync<PaddedInputHead, warpTile.y, ctaShapeInWarps.x, grainBytes, grainBytes, qkSwizzle, false,
                warpTile.y>(warpIdx.x, smem.q[warpIdx.y][0], src, nbValidHeadTokens, localQHeadTokenIdxMap);
        }
    ...
    if (warpIdx.z == 0)
    {
        ...
        static_assert(nbQBuffers == 1);
        copyHeadsAsync<PaddedInputHead, warpTile.y, ctaShapeInWarps.x, grainBytes, grainBytes, qkSwizzle, isFullTile,
            warpTile.y>(warpIdx.x, smem.q[warpIdx.y][0], src, nbValidRows, localQHeadIdxMap);
        ldgsts::barArrive(smem.qBarrier[warpIdx.y], true);
    }
#endif
    ...
    if (warpIdx.z == 0)
    {
        float const qkScale = qScaleValue * (isKCacheQuantized ? kCacheScaleValue : 1.f)
...
#if ENABLE_4BIT_K_CACHE
        auto const getSMemKSfTile = [&](uint32_t idx) -> SharedMem::KSfSmemBuffer& { return smem.kSf[warpIdx.x][idx]; };
#endif
#if BEAM_WIDTH > 1
        ...
        auto loadKTilePart = [&](uint32_t seqIter, uint32_t idxBeam, uint32_t idxPart) mutable
        {
            ...
            HeadPtr<GMemKCacheHead const, tokensPerPage, nbPagesPerWarpTile> const src{
                cacheList.kCacheVLLM, pageIdx, tokenOffset, idxHeadGrp, k_stride_page, k_stride_token, k_stride_head};
#if ENABLE_4BIT_K_CACHE
            HeadPtr<GMemKCacheHeadSf const, tokensPerPage, nbPagesPerWarpTile> const srcSf{cacheList.kSfCacheVLLM,
                pageIdx, tokenOffset, idxHeadGrp, k_sf_stride_page, k_sf_stride_token, k_sf_stride_head};
#endif
            ...
            if (isFullTile)
            {
                copyPartialHeadsAsync<PaddedKCacheHead, warpTile.x, nbPartsPerCacheKHead, grainBytes,
                    grainBytesGmemKCache, qkSwizzle, true>(warp, dst, dstHeadOffset, src, idxPart);
            }
            else
            {
                ...
                copyPartialHeadsAsync<PaddedKCacheHead, warpTile.x, nbPartsPerCacheKHead, grainBytes,
                    grainBytesGmemKCache, qkSwizzle, false>(warp, dst, dstHeadOffset, src, idxPart, nbHeadsAvail);
            }
#if ENABLE_4BIT_K_CACHE
            copyPartialHeadsAsync<PaddedKCacheHeadSf, warpTile.x, nbPartsPerCacheKHead, grainBytesSf, grainBytesSf,
                false, true>(warp, dstSf, dstHeadOffset, srcSf, idxPart);
#endif
...
#else
                        WarpAcc tmpAcc{};
                        smemXVPartGemm<VCacheElem>(warp, tmpAcc, skipXRowRescale, xRowNeedRescaleMask, xRowScales,
                            smemXTile, idxVTile, smemVTile,
#if ENABLE_4BIT_V_CACHE
                            smemVSfPart,
#endif
...
#if SPEC_DEC
                addAttentionSinksSpecDec(
                    globalRowSum, globalRowMax, attentionSinks + headGrpSize * idxHeadGrp, headGrpSize);
#else
...
#if SPEC_DEC
                    addAttentionSinksSpecDec(
                        mergedRowSum, mergedRowMax, attentionSinks + headGrpSize * idxHeadGrp, headGrpSize);
#else
    ...
    uint32_t* __restrict__ semaphores = nullptr, void* __restrict__ scratch = nullptr)
{
    ...
    kernel_mha_impl(nbKHeads,
        ...
        beamSearchParams,
#endif
        batchSize, kCacheScale, kScalePtr, vCacheScale, vScalePtr, k_stride_page, k_stride_token, k_stride_head,
        v_stride_page, v_stride_token, v_stride_head, k_sf_stride_page, k_sf_stride_token, k_sf_stride_head,
        v_sf_stride_page, v_sf_stride_token, v_sf_stride_head, semaphores, scratch);
}
#else
...
void launchMHA(cudaDeviceProp const& prop, uint32_t nbKHeads,
    ...
    BeamSearchParams const& beamSearchParams,
#endif
    uint32_t batchSize, float kCacheScale, float const* kScalePtr, float vCacheScale, float const* vScalePtr,
#if SPEC_DEC
    SpecDecParams const& specDecParams,
#endif
    uint32_t* semaphores, void* scratch, bool enable_pdl, uint64_t k_stride_page, uint64_t k_stride_token,
    uint64_t k_stride_head, uint64_t v_stride_page, uint64_t v_stride_token, uint64_t v_stride_head,
    uint64_t k_sf_stride_page, uint64_t k_sf_stride_token, uint64_t k_sf_stride_head, uint64_t v_sf_stride_page,
    uint64_t v_sf_stride_token, uint64_t v_sf_stride_head, cudaStream_t stream)
{
...
#if SPEC_DEC
    uint32_t const nbTokenBlocksPerGrp = divUp(qSeqLen * headGrpSize, rowsPerBlock);
    ...
    uint32_t const k_container_elems_per_head = validElemsPerHead / KCacheElemConverter::ElemsPerContainer;
    uint32_t const v_container_elems_per_head = validElemsPerHead / VCacheElemConverter::ElemsPerContainer;
    uint32_t const k_stride_page_in_heads = static_cast<uint32_t>(k_stride_page / k_container_elems_per_head);
    uint32_t const k_stride_token_in_heads = static_cast<uint32_t>(k_stride_token / k_container_elems_per_head);
    uint32_t const k_stride_head_in_heads = static_cast<uint32_t>(k_stride_head / k_container_elems_per_head);
    uint32_t const v_stride_page_in_heads = static_cast<uint32_t>(v_stride_page / v_container_elems_per_head);
    uint32_t const v_stride_token_in_heads = static_cast<uint32_t>(v_stride_token / v_container_elems_per_head);
    uint32_t const v_stride_head_in_heads = static_cast<uint32_t>(v_stride_head / v_container_elems_per_head);
    uint32_t const k_sf_stride_page_in_heads
        = static_cast<uint32_t>(k_sf_stride_page / (validElemsPerHead / KCacheElemConverter::QuantVectorSize));
    uint32_t const k_sf_stride_token_in_heads
        = static_cast<uint32_t>(k_sf_stride_token / (validElemsPerHead / KCacheElemConverter::QuantVectorSize));
    uint32_t const k_sf_stride_head_in_heads
        = static_cast<uint32_t>(k_sf_stride_head / (validElemsPerHead / KCacheElemConverter::QuantVectorSize));
    uint32_t const v_sf_stride_page_in_heads
        = static_cast<uint32_t>(v_sf_stride_page / (validElemsPerHead / VCacheElemConverter::QuantVectorSize));
    uint32_t const v_sf_stride_token_in_heads
        = static_cast<uint32_t>(v_sf_stride_token / (validElemsPerHead / VCacheElemConverter::QuantVectorSize));
    uint32_t const v_sf_stride_head_in_heads
        = static_cast<uint32_t>(v_sf_stride_head / (validElemsPerHead / VCacheElemConverter::QuantVectorSize));
    ...
    cudaLaunchKernelEx(&launchCfg, kernel_mha,
        ...
        beamSearchParams,
#endif
        batchSize, kCacheScale, kScalePtr, vCacheScale, vScalePtr, k_stride_page_in_heads, k_stride_token_in_heads,
        k_stride_head_in_heads, v_stride_page_in_heads, v_stride_token_in_heads, v_stride_head_in_heads,
        k_sf_stride_page_in_heads, k_sf_stride_token_in_heads, k_sf_stride_head_in_heads, v_sf_stride_page_in_heads,
...
void launchMHAFlashInfer(uint32_t multiProcessorCount, uint32_t nbKHeads, uint32_t slidingWinSize, float qScale,
    float const* qScalePtr, OutputHead* output,
#if LOW_PREC_OUTPUT
    float rcpOutScale,
#endif
    InputHead const* q, float const* attentionSinks, GMemKCacheHead* kCacheVLLM, GMemVCacheHead* vCacheVLLM,
#if ENABLE_4BIT_K_CACHE
    GMemKCacheHeadSf* kSfCacheVLLM,
#endif
#if ENABLE_4BIT_V_CACHE
    GMemVCacheHeadSf* vSfCacheVLLM,
#endif
    KVCachePageIndex const* kvCachePageList, uint32_t maxSeqLen, uint32_t const* seqLen, uint32_t batchSize,
    float kCacheScale, float const* kScalePtr, float vCacheScale, float const* vScalePtr,
#if SPEC_DEC
    ...
    uint64_t v_sf_stride_token, uint64_t v_sf_stride_head, cudaStream_t stream)
{
    uint32_t const nbSubSeqPerSeq = [&]() -> uint32_t
    {
        ...
        return std::min<uint32_t>(
            std::max<uint32_t>(1U, multiProcessorCount / (batchSize * nbKHeads)), divUp(maxSeqLen, ctaTile.x));
    }();
#if SPEC_DEC
    uint32_t const nbTokenBlocksPerGrp = divUp(qSeqLen * headGrpSize, rowsPerBlock);
    dim3 const dimGrid{nbSubSeqPerSeq, nbKHeads * nbTokenBlocksPerGrp, batchSize};
    ...
    uint32_t const v_stride_token_in_heads = static_cast<uint32_t>(v_stride_token / v_container_elems_per_head);
    uint32_t const v_stride_head_in_heads = static_cast<uint32_t>(v_stride_head / v_container_elems_per_head);
    uint32_t const k_sf_stride_page_in_heads = static_cast<uint32_t>(k_sf_stride_page / k_sf_elems_per_head);
    uint32_t const k_sf_stride_token_in_heads = static_cast<uint32_t>(k_sf_stride_token / k_sf_elems_per_head);
    uint32_t const k_sf_stride_head_in_heads = static_cast<uint32_t>(k_sf_stride_head / k_sf_elems_per_head);
    uint32_t const v_sf_stride_page_in_heads = static_cast<uint32_t>(v_sf_stride_page / v_sf_elems_per_head);
    uint32_t const v_sf_stride_token_in_heads = static_cast<uint32_t>(v_sf_stride_token / v_sf_elems_per_head);
    uint32_t const v_sf_stride_head_in_heads = static_cast<uint32_t>(v_sf_stride_head / v_sf_elems_per_head);
    ...
    cudaLaunchKernelEx(&launchCfg, kernel_mha,
        ...
        mask,
#endif
        attentionSinks, cacheList, batchSize, kCacheScale, kScalePtr, vCacheScale, vScalePtr, k_stride_page_in_heads,
        k_stride_token_in_heads, k_stride_head_in_heads, v_stride_page_in_heads, v_stride_token_in_heads,
        v_stride_head_in_heads, k_sf_stride_page_in_heads, k_sf_stride_token_in_heads, k_sf_stride_head_in_heads,
        v_sf_stride_page_in_heads, v_sf_stride_token_in_heads, v_sf_stride_head_in_heads, semaphores, scratch);
    checkCuda(cudaPeekAtLastError());
}
