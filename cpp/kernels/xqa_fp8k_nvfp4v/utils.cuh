...
#elif __CUDA_ARCH__ == 900 || __CUDA_ARCH__ == 1000 || __CUDA_ARCH__ == 1030 || __CUDA_ARCH__ == 1100
...
__device__ inline bool hasBankConflict(T* p)
{
    ...
    uint32_t const grpMask = (((1U << grpSize) - 1U) << (laneId() / grpSize * grpSize));
...
template <>
__device__ inline Vec<uint32_t, 2> convertKCacheWordToF16<__nv_bfloat16, __nv_fp8_e4m3>(uint32_t i8data)
{
    Vec<uint32_t, 2> fp16;
#if (defined __CUDA_ARCH__) && (__CUDA_ARCH__ >= 890)
    uint16_t (&src)[2] = reinterpret_cast<uint16_t (&)[2]>(i8data);
...
template <>
__device__ inline Vec<uint32_t, 2> convertKCacheWordToF16<__nv_bfloat16, __nv_fp4_e2m1>(uint32_t i8data)
{
    Vec<uint32_t, 2> ret;
...
template <>
__device__ inline Vec<uint32_t, 2> convertVCacheWordToF16<__nv_bfloat16, __nv_fp8_e4m3>(uint32_t i8data)
{
    Vec<uint32_t, 2> fp16;
#if (defined __CUDA_ARCH__) && (__CUDA_ARCH__ >= 890)
    asm("{\n"
