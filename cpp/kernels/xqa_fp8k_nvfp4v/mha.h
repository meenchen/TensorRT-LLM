...
static_assert((isMLA || validElemsPerHead <= 256) && (sizeof(KCacheElem) * validElemsPerHead) % 16 == 0
...
using GMemKCacheHead
    = Vec<KCacheElemConverter::ContainerType, exactDiv(validElemsPerHead, KCacheElemConverter::ElemsPerContainer)>;
using GMemVCacheHead
    = Vec<VCacheElemConverter::ContainerType, exactDiv(validElemsPerHead, VCacheElemConverter::ElemsPerContainer)>;
using GMemCacheHead = GMemVCacheHead;
#if ENABLE_4BIT_K_CACHE
using GMemKCacheHeadSf
    = Vec<KCacheElemConverter::ScalingFactorType, exactDiv(validElemsPerHead, KCacheElemConverter::QuantVectorSize)>;
#endif
#if ENABLE_4BIT_V_CACHE
using GMemVCacheHeadSf
    = Vec<VCacheElemConverter::ScalingFactorType, exactDiv(validElemsPerHead, VCacheElemConverter::QuantVectorSize)>;
#endif
...
void launchHopperF8MHAFlashInfer(uint32_t multiProcessorCount, uint32_t nbKHeads, uint32_t slidingWinSize, float qScale,
    float const* qScalePtr, OutputHead* output,
#if LOW_PREC_OUTPUT
    float rcpOutScale,
#endif
    InputHead const* q, float const* attentionSinks, GMemCacheHead* kCacheVLLM, GMemCacheHead* vCacheVLLM,
    KVCachePageIndex const* kvCachePageList, uint32_t maxSeqLen, uint32_t const* seqLen, uint32_t batchSize,
    float kvCacheScale, float const* kvScalePtr,
#if SPEC_DEC
    uint32_t qSeqLen, uint32_t const* qCuSeqLens, MaskType const* mask,
#endif
    uint32_t* semaphores, void* scratch, bool enable_pdl, uint64_t kv_stride_page, uint64_t kv_stride_token,
...
void launchMLA(cudaDeviceProp const& prop,
    ...
    float qScale, float const* qScalePtr, OutputHead* output, InputHead const* q, GMemCacheHead* kCacheVLLM,
