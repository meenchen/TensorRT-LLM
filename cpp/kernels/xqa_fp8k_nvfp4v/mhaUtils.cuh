...
struct IndexedHeadPtrImpl
{
    ...
    __device__ inline Head* operator+(uint32_t i) const
    {
        ...
        return pool + pageIdx * stride_page + (absoluteTokenIdx % tokensPerPage) * stride_token + headIdx * stride_head;
    }
};
...
struct HeadPtr
{
    ...
    __device__ inline Head* operator+(uint32_t i) const
    {
        ...
        return (pageIdx & (1U << 31))
            ? nullptr
            : pool + pageIdx * stride_page + (absoluteTokenIdx % tokensPerPage) * stride_token + headIdx * stride_head;
    }
};
...
template <typename Head, uint32_t maxNbCopiedHeads, uint32_t nbPartsPerHead, uint32_t grainBytesSmem,
    uint32_t grainBytesGmem, bool swizzle, bool isFull, uint32_t dstNbHeads, typename SrcHeadPtr, typename _LdGrain,
    typename LocalHeadIdxMap = uint32_t (*)(uint32_t)>
__device__ inline void copyPartialHeadsAsync(
    Warp const& warp,
    Array2D<_LdGrain, dstNbHeads, exactDiv(exactDiv(sizeof(Head), nbPartsPerHead), grainBytesSmem)>& dst,
...
template <typename Head, uint32_t maxNbCopiedHeads, uint32_t nbWarps, uint32_t grainBytesSmem, uint32_t grainBytesGmem,
    bool swizzle, bool isFull, uint32_t dstNbHeads, typename SrcHeadPtr, typename _LdGrain,
    typename LocalHeadIdxMap = uint32_t (*)(uint32_t)>
__device__ inline void copyHeadsAsync(
    uint32_t idxWarp, Array2D<_LdGrain, dstNbHeads, exactDiv(sizeof(Head), grainBytesSmem)>& dst, SrcHeadPtr const& src,
    uint32_t nbAvailHeads = maxNbCopiedHeads, LocalHeadIdxMap&& localHeadIdxMap = [](uint32_t x) { return x; })
{
    ...
    copyPartialHeadsAsync<Head, maxNbHeadsPerWarp, 1, grainBytesSmem, grainBytesGmem, swizzle, isFull, dstNbHeads>(warp,
        dst, dstHeadOffset, src, idxPart, warpNbAvailHeads,
